...
Regarding the \textit{is\_valid\_addropt\_opsize()} function, it has been developed as a separate inline function called inside the ``if'' statement for better readability, since the length's check with the additional ADD\_ADDR2 case involves now four possible configurations and it is quite verbose.
...
A separate Python script has been developed together with gnuplot in order to track and visualize the local CPU usage of the client virtual machine process over time \cite{monitor}. The tool simply samples the CPU usage percentage for the UML process from the command \textit{top}.
