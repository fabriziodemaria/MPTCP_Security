...
The \textit{crypto\_hash} structure initialized in the function \textit{tcp\_init\_sock()} is needed by the subsequent Crypto functions present in \textit{mptcp\_hmac\_sha1()}; it is saved in the ``per\_cpu'' structure called \textit{mptcp\_hmacsha1\_pool} and it is retrieved from the same structure as shown in line 11 of listing \ref{crypto}. The ``per\_cpu'' usage allows for better performance in a multi-core environment.
...
It is possible to notice that the code inside \textit{mptcp\_handle\_add\_addr()} is similar to the one used at the sender (\ref{fullmesh}), but in this case, in order to computer the same HMAC value, the local key used for HMAC computation is substituted with remote key (from the perspective of the receiver); moreover, for this code that runs at the receiver, the port value is present and processed if detected inside the ADD\_ADDR(2) option, as it explained later in the following sections on port advertisement and the new hashing function's implementation.
