...
\begin{itemize}
    ...
    \item Evaluating (or developing) proper external tools to monitor and observe and capture all the low-level details of the packets exchanged during the MPTCP sessions;
    \item Evaluating (or developing) proper external tools to easily operate on the ongoing MPTCP sessions, by manipulating and injecting forged packets. Other ways to operate over the connections have been taken into consideration as well, like the firewall setup necessary to block specific kind of packets.
\end{itemize}
...
\begin{itemize}
    ...
    \item Applying a relatively small amount of changes to the current implementation according to the previous step;
    \item Extensively testing all the regular and edge cases potentially affected by the modifications elaborated in the previous step (by using the tools from phase 1);
    \item Restarting the loop from the first step in case of malfunctioning due to bug in the code; otherwise, changes were pushed into experimental repository for external evaluations, reviews and feedbacks.
\end{itemize}
...
Each one of the features have been developed starting from the specifications and comments from external sources, and the collaborative working environment with the other stakeholders involved in the project allowed to reverse the operational flow and occasionally propose more or less formal modifications regarding the protocol's functioning.
